...
var packetRoutingTCP = {
	...
	"question_correct_answer": funcref(self, "question_correct_answer_handle"),
...
func tcp_packet_handle():
	while socketTCPWrapper.get_available_packet_count() > 0:
		var string = socketTCPWrapper.get_packet().get_string_from_ascii()
...
func connection_want_send():
	...
	var packetStr = JSON.print(packet)
	socketTCPWrapper.put_packet(packetStr.to_ascii())
	
	# Log
	self.add_console_line("[TCP] Sent 'connection_want' message.")	
	return true
...
func info_update_send(nick, icon_id):
	...
	# Log
	self.add_console_line("[TCP] Sent 'info_update' message.")
	return true
...
func give_answer_send(question_number, answer_number):
	...
	# Log
	self.add_console_line("[TCP] Sent 'give_answer' message.")
	return true
...
func player_ready_send():
	...
	# Log
	self.add_console_line("[TCP] Sent 'player_ready' message.")
	return true
...
func i_am_lost_send(player_id):
	...
	# Log
	self.add_console_line("[TCP] Sent 'i_am_lost' message.")
	return true
...
func _send_packet_helper(packet):
	...
	socketTCPWrapper.put_packet(PacketString.to_ascii())
...
func question_send_handle(dict):
	var fields = ['question_number', 'texts']
...
func question_correct_answer_handle(dict):
	var fields = ['question_number', 'correct_answer_number']
